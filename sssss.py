...
# === NEW METHOD: DETECT CURVE SHAPE ===
def get_lane_shape(lines_y, lines_w, cx):
    """
    Returns:
    - center_bottom: Lane center at the feet of the robot (Current Position)
    - center_top: Lane center at the horizon (Lookahead)
    - curve_val: The difference (Positive = Right Curve, Negative = Left Curve)
    """
    fits_y = fit_lines(lines_y)
    fits_w = fit_lines(lines_w)

    # Evaluation points: Bottom (near robot) and Top (horizon)
    y_bot = FRAME_H - 5
    y_top = ROI_TOP + 20

    # 1. Find Yellow (Right) at Bottom and Top
    ...
    # 2. Find White (Left) at Bottom and Top
    ...
    elif x_y_bot: center_bot = x_y_bot - (LANE_WIDTH_PX * 0.5) # Hug Right
    elif x_w_bot: center_bot = x_w_bot + (LANE_WIDTH_PX * 0.5)

    # Calculate Top Center (Lookahead)
...
def main():
    ...
    try:
        while True:
            ...
            # *** NEW: Get Lane Shape (Lookahead) ***
            lane_center, curve_val = get_lane_shape(lines_y, lines_w, FRAME_W*0.5)
            ...
            mio = get_mio_in_lane(tracks, None, None)

            # 4. CONTROL LOGIC WITH CURVE FOLLOWING
            speed, turn = BASE_SPEED, 0.0

            if lane_center is not None:
                # A. Base Steering (Where am I now?)
                err_pos = (FRAME_W/2) - lane_center
                steer_pos = lane_pd.step(err_pos)

                # B. Shape Following (Where is the road going?)
                # If curve_val is + (Right turn), we add + steering (Right).
                steer_curve = curve_val * K_CURVE

                # C. Combine them
                total_steer_lane = steer_pos + steer_curve

                # D. Turn Detection Logic (For display/speed)
                turn_status = "STRAIGHT"
                if curve_val > 50: 
                    turn_status = "RIGHT TURN DETECTED ➡️"
                    speed -= CURVE_SLOW_DOWN # Slow down for turn
                elif curve_val < -50: 
                    turn_status = "LEFT TURN DETECTED ⬅️"
                    speed -= CURVE_SLOW_DOWN
                ...
                turn = np.clip(total_steer_lane + steer_avoid, -MAX_TURN, MAX_TURN)
                ...
                # print(f"Shape: {turn_status} | Curve: {int(curve_val)}")
